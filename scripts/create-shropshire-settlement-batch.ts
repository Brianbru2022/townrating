...
const cacheDirectory = resolve('tmp/shropshire-settlement-batch-v1');
...
const broadBboxes = [
  ...
  [52.3, -2.85, 52.6, -2.45],
  [52.3, -2.45, 52.6, -2.1],
  [52.6, -3.25, 52.85, -2.85],
  [52.6, -2.85, 52.85, -2.45],
  [52.6, -2.45, 52.85, -2.1],
  [52.85, -3.25, 53.1, -2.85],
  [52.85, -2.85, 53.1, -2.45],
  [52.85, -2.45, 53.1, -2.1],
] as const;
...
interface SettlementInventoryEntry {
  ...
  osmId: number;
  sourceUrl: string;
  authority: 'Shropshire' | 'Telford and Wrekin';
  authorityCode: 'E06000051' | 'E06000020';
}
...
const researchedParkingFacts = new Map<string, ResearchedParkingFact>([
  ['way-655978446', {
    paymentRequired: 'no',
    priceDisplay: 'Free',
    sourceName: 'Victoria Park visitor information',
    sourceOrganisation: 'Telford & Wrekin Council',
    sourceRecordId: 'newport-water-lane-car-park',
    sourceUrl: 'https://www.telford.gov.uk/sports-and-leisure/green-and-open-spaces/parks/victoria-park/',
    notes: 'The council visitor page identifies the Water Lane car park as free.',
  }],
  ['way-1532851189', {
    paymentRequired: 'no',
    priceDisplay: 'Free',
    sourceName: 'Hartshill Park visitor information',
    sourceOrganisation: 'Telford & Wrekin Council',
    sourceRecordId: 'oakengates-bridge-street-long-stay',
    sourceUrl: 'https://www.telford.gov.uk/sports-and-leisure/green-and-open-spaces/parks/hartshill-park/',
    notes: 'The council visitor page identifies Bridge Street long-stay car park as free.',
  }],
  ['way-128383993', {
    paymentRequired: 'no',
    priceDisplay: 'Free parking; Park & Ride bus fare is separate',
    sourceName: 'Ludlow Park & Ride',
    sourceOrganisation: 'Shropshire Council',
    sourceRecordId: 'ludlow-eco-park-and-ride',
    sourceUrl: 'https://next.shropshire.gov.uk/public-transport/park-and-ride/ludlow-park-and-ride/',
    notes: 'The council states that parking at the Eco Park & Ride is free; the bus fare is charged separately.',
  }],
  ['way-368119393', {
    paymentRequired: 'yes',
    priceDisplay: 'Paid Telford Centre parking; check the current tariff',
    sourceName: 'Telford Centre parking tariffs',
    sourceOrganisation: 'Telford Centre',
    sourceRecordId: 'telford-centre-parking-tariffs',
    sourceUrl: 'https://www.telfordcentre.com/visit/parking/',
    notes: 'This mapped Telford Centre car park is paid. Current tariffs and car-park naming should be checked before travel.',
  }],
]);
...
function routeMeta(html: string, label: string) {
  const match = html.match(new RegExp(`<div class="sgo-meta [^"]*"><h2>${label}<\\/h2><span>([\\s\\S]*?)<\\/span><\\/div>`, 'i'));
  return match ? stripHtml(match[1]) : undefined;
}

async function fetchShropshireTrails() {
  const cachePath = resolve(cacheDirectory, 'shropshire-great-outdoors-routes.json');
  try {
    const cached = JSON.parse(await readFile(cachePath, 'utf8')) as ResearchedTrail[];
    return cached.map((trail) => ({ ...trail, title: stripHtml(trail.title) }));
  } catch {
    // Fetch and cache below.
  }
  const sitemapIndex = await (await fetch('https://www.shropshiresgreatoutdoors.co.uk/wp-sitemap.xml', {
    headers: { 'User-Agent': 'TownscapeGuides/1.0 (curated route audit)' },
    signal: AbortSignal.timeout(45_000),
  })).text();
  const sitemapUrls = [...sitemapIndex.matchAll(/<loc>(https:\/\/www\.shropshiresgreatoutdoors\.co\.uk\/route-sitemap\d*\.xml)<\/loc>/g)].map((match) => match[1]);
  const routeUrls = new Set<string>();
  for (const sitemapUrl of sitemapUrls) {
    const xml = await (await fetch(sitemapUrl, {
      headers: { 'User-Agent': 'TownscapeGuides/1.0 (curated route audit)' },
      signal: AbortSignal.timeout(45_000),
    })).text();
    for (const match of xml.matchAll(/<loc>(https:\/\/www\.shropshiresgreatoutdoors\.co\.uk\/route\/[^<]+)<\/loc>/g)) routeUrls.add(match[1]);
  }
  const urls = [...routeUrls];
  const routes: ResearchedTrail[] = [];
  const workers = Array.from({ length: 8 }, async (_, worker) => {
    for (let index = worker; index < urls.length; index += 8) {
      ...
      try {
        ...
        const nearest = routeMeta(html, 'Nearest towns?');
        if (!title || !nearest) continue;
        const nearestTowns = nearest.split(/\s*(?:,|\/| and )\s*/i).map((value) => value.trim()).filter(Boolean);
        const walking = /(?:>Walking<|sgo-icon-walk|\bwalk(?:ing)?\b)/i.test(html) && !/\bcycle rides?\b/i.test(title);
        if (!walking) continue;
        routes.push({
          title: title.replace(/\s+-\s+Shropshire's Great Outdoors.*$/i, ''),
          url,
          nearestTowns,
          distance: routeMeta(html, 'Distance'),
          duration: routeMeta(html, 'Duration'),
          difficulty: routeMeta(html, 'Difficulty'),
          circular: /sgo-icon-circular/i.test(html),
          historic: /sgo-icon-history/i.test(html),
          walking,
        });
      } catch {
        // An unavailable route is omitted rather than represented from incomplete evidence.
      }
    }
  });
  await Promise.all(workers);
  routes.sort((left, right) => left.title.localeCompare(right.title, 'en-GB'));
  await writeFile(cachePath, `${JSON.stringify(routes, null, 2)}\n`, 'utf8');
  return routes;
}
...
function treasureMatch(locality: string, products: TreasureProduct[]) {
  ...
  const allowedLocations = new Set(['Shropshire', 'Telford and Wrekin', 'Telford'].map(normalise));
...
const researchedAttractionScores: Record<string, Record<string, number>> = {
  ludlow: {
    'ludlow castle': 92,
    'church of st laurence and attached railings': 85,
    'ludlow museum': 82,
    'the butter cross': 78,
  },
  shrewsbury: {
    'ditherington flax mill': 92,
    'former her majestys prison shrewsbury': 88,
    'shrewsbury museum the music hall': 86,
    'shrewsbury castle': 84,
    'abbey church of the holy cross': 82,
    'soldiers of shropshire museum': 80,
    'old market hall': 78,
  },
  telford: {
    'blists hill victorian town': 94,
    'coalbrookdale museum of iron': 90,
    enginuity: 88,
    'jackfield tile museum': 86,
    'the iron bridge tollhouse': 85,
    'museum of the gorge': 84,
    'the darby houses': 82,
  },
  ironbridge: {
    'the iron bridge': 94,
    'the iron bridge tollhouse': 84,
    'bedlam furnaces': 78,
  },
  'much wenlock': {
    'priory of st milburga ruins': 88,
    'much wenlock museum': 80,
    guildhall: 78,
  },
  bridgnorth: {
    'upper station of cliff railway': 88,
    'bridgnorth castle': 82,
    'northgate museum': 78,
    'town hall': 75,
  },
  oswestry: {
    'oswestry town museum': 78,
    'cambrian railways museum': 76,
  },
  whitchurch: {
    'whitchurch heritage centre': 78,
  },
  'bishops castle': {
    'the house on crutches museum': 78,
  },
};
...
const researchedAttractionExclusions: Record<string, Set<string>> = {
  ...
  shrewsbury: new Set([
    ...
    'shrewsbury sixth form college the priory',
    'shrewsbury flaxmill maltings cross building',
  ]),
  'much wenlock': new Set([
    'priory tower',
    'the priory',
    'wenlock abbey',
  ]),
  bridgnorth: new Set(['the castle']),
  oswestry: new Set(['remains of oswestry castle']),
  whitchurch: new Set(['old town hall vaults']),
  'bishops castle': new Set([
...
function regionFor(locality: string) {
  return regionByLocality.get(normalise(locality)) ?? 'Shropshire';
}
...
async function buildTown(
  ...
  const projectId = `${slug}-shropshire-england`;
  ...
  for (const [index, trail] of officialTrails.entries()) {
    ...
    const feature = createOsmFeature(projectId, locality, trailElement, 'trails', trail.title, `An official Shropshire Great Outdoors walking route associated with ${locality}.`, {
    ...
    feature.sourceRecords = [feature.sourceRecords[0], source(
      "Shropshire's Great Outdoors route page",
      'Shropshire Council',
  ...
  const visitorHighlights = attractions.map((item, index) => ({
    ...
    timeToSpend: recommendedAttractionDuration(item.feature, item.score),
  ...
  const packageData: ProjectPackage = {
    project: {
      ...
      townGuide: {
        ...
        perfectFor: rating === 0 ? ['Visitors already nearby'] : ['A short local detour', 'Visitors exploring Shropshire'],
        suggestedFirstVisit: {
...
async function writeGeneratedModule(projectIds: string[]) {
  ...
  const contents = `// Generated by scripts/create-shropshire-settlement-batch.ts. Do not edit by hand.\nimport type { ProjectPackage } from '../domain/models';\n${imports}\n\nexport const shropshireSettlementPackages: ProjectPackage[] = [\n${items}\n];\n`;
...
await writeFile(resolve(reviewDirectory, `shropshire-settlement-batch-audit-${reviewedDate}.json`), `${JSON.stringify(report, null, 2)}\n`, 'utf8');
