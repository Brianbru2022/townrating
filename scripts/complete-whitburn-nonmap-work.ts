...
const projectPath = resolve(process.argv[2] ?? 'data/projects/whitburn.json');
const completionReviewPath = resolve(process.argv[3] ?? 'data/review/whitburn-completion-review.json');
const linkedAuditPath = resolve('data/review/whitburn-linked-source-audit.json');
const currentPlaceCurationPath = resolve('data/curation/whitburn-current-place-curation.json');
...
function applyDate(feature: HeritageFeature, evidence: DateEvidence): void {
  ...
  addTags(feature, 'date-reviewed', 'curation-date-enriched', 'whitburn-nonmap-date-reviewed');
  appendReviewNote(
    feature,
    'Whitburn completion review: broad date evidence has been normalised from the linked official HES/NRHE record.',
  );
}
...
function markNoDate(feature: HeritageFeature): void {
  feature.reviewed = true;
  feature.updatedAt = reviewedAt;
  addTags(feature, 'date-reviewed', 'reviewed-no-defensible-date', 'whitburn-date-reviewed-no-date');
  appendReviewNote(
    feature,
    'Whitburn completion review: the official source was checked and no defensible construction or historic-period date is published for this record.',
  );
}
...
function currentSummary(feature: HeritageFeature): string {
  ...
  if (generic.has(feature.name)) return `Current ${label} in Whitburn recorded by OpenStreetMap.`;
  return `${feature.name} is a current ${label} in Whitburn recorded by OpenStreetMap.`;
}
...
for (const feature of pkg.features) {
  if (!feature.documentedDateText && (feature.id.startsWith('nrhe:') || feature.id.startsWith('hes-'))) {
    const evidence = evidenceFromClassification(feature);
    if (evidence) {
...
const linkedSourceAudit = pkg.features
  ...
  .map(({ feature, kinds }) => {
    feature.reviewed = true;
    feature.updatedAt = reviewedAt;
    addTags(feature, 'whitburn-linked-source-reviewed');
    appendReviewNote(
      feature,
  ...
  `${JSON.stringify(
    {
      ...
      reviewedAt,
      policy:
        'Completed Whitburn non-map curation. Explicit HES/NRHE period evidence was normalised conservatively; period-unknown, period-unassigned, event and general-view records were marked reviewed without inventing dates.',
      dated,
      reviewedWithoutDate,
      currentPlaceReview: {
...
console.log(
  `Completed Whitburn non-map work: ${dated.length} date review(s), ${reviewedWithoutDate.length} reviewed-without-date record(s), ${currentPlaceEntries.length} current place(s), ${currentParksReviewed} current park(s), ${linkedSourceAudit.length} linked-source audit record(s).`,
);
