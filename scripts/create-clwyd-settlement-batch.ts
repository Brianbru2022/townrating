...
const cacheDirectory = resolve('tmp/clwyd-settlement-batch-v1');
...
const broadBboxes = [
  [52.75, -4.15, 53.08, -3.55],
  [52.75, -3.55, 53.08, -3.15],
  [52.75, -3.15, 53.08, -2.75],
  [53.08, -4.15, 53.42, -3.55],
  [53.08, -3.55, 53.42, -3.15],
  [53.08, -3.15, 53.42, -2.75],
] as const;
const regionalExtent = { south: 52.75, west: -4.15, north: 53.42, east: -2.75 };
...
async function fetchNominatimBoundary(locality: string) {
  ...
  const result = results
    ...
    .map((candidate) => ({
      ...
      score: (normalise(candidate.address?.village ?? candidate.address?.town ?? candidate.address?.hamlet ?? candidate.address?.suburb ?? '') === requested ? 5 : 0)
        + (/(?:denbighshire|sir ddinbych|flintshire|sir y fflint|wrexham|wrecsam|conwy|wales|cymru)/i.test(candidate.display_name) ? 3 : 0)
  ...
  if (!result) throw new Error(`No OSM settlement result inside the Clwyd regional extent for ${locality}`);
  ...
  for (const [folder, designationType, tag, sourceKind] of folders) {
    ...
    for (const filename of (await readdir(directory)).filter((name) => name.endsWith('.geojson'))) {
      ...
      for (const record of collection.features) {
        ...
        if (!/(?:conwy|denbighshire|flintshire|wrexham|powys|shropshire)/i.test(authority)) continue;
...
function treasureMatch(locality: string, products: TreasureProduct[]) {
  ...
  const allowedLocations = new Set(['Clwyd', 'North Wales', 'Wales'].map(normalise));
...
const researchedAttractionScores: Record<string, Record<string, ResearchedAttraction>> = {
  Conwy: {
    [normalise('Conwy Castle')]: { score: 94, sourceUrl: 'https://cadw.gov.wales/visit/places-to-visit/castell-conwy' },
    [normalise('Conwy Town Walls')]: { score: 90, sourceUrl: 'https://cadw.gov.wales/visit/places-to-visit/conwy-town-walls', freeAdmission: true },
    [normalise('Plas Mawr')]: { score: 87, sourceUrl: 'https://cadw.gov.wales/visit/places-to-visit/plas-mawr' },
    [normalise('Aberconwy House')]: { score: 82, sourceUrl: 'https://www.nationaltrust.org.uk/visit/wales/aberconwy-house' },
    [normalise('The Smallest House in Great Britain')]: { score: 78, sourceUrl: 'https://www.visitconwy.org.uk/listing/the-smallest-house-in-great-britain/27717101/' },
  },
  Llandudno: {
    [normalise('Great Orme Visitor Centre')]: { score: 90, sourceUrl: 'https://www.visitconwy.org.uk/listing/great-orme-country-park-and-local-nature-reserve/27719101/' },
    [normalise('Great Orme Tramway')]: { score: 89, sourceUrl: 'https://greatormetramway.co.uk/' },
    [normalise('Great Orme Bronze Age Mines')]: { score: 88, sourceUrl: 'https://www.greatormemines.info/' },
    [normalise('Llandudno Pier')]: { score: 87, sourceUrl: 'https://www.visitconwy.org.uk/listing/llandudno-pier/27751101/', freeAdmission: true },
    [normalise('Llandudno Museum')]: { score: 78, sourceUrl: 'https://llandudnomuseum.co.uk/' },
  },
  Llanrwst: {
    [normalise('Pont Fawr')]: { score: 78, sourceUrl: 'https://www.visitconwy.org.uk/explore/towns-and-villages/llanrwst/', freeAdmission: true },
    [normalise("Tu Hwnt i'r Bont")]: { score: 76, sourceUrl: 'https://www.visitconwy.org.uk/explore/towns-and-villages/llanrwst/' },
  },
  Llangollen: {
    [normalise('Llangollen Railway')]: { score: 89, sourceUrl: 'https://llangollen-railway.co.uk/' },
    [normalise('Llangollen Wharf Horse Drawn Boat Trips')]: { score: 86, sourceUrl: 'https://www.horsedrawnboats.co.uk/' },
    [normalise('Plas Newydd')]: { score: 84, sourceUrl: 'https://www.denbighshire.gov.uk/en/leisure-and-tourism/museums-and-historic-houses/plas-newydd-llangollen.aspx' },
    [normalise('Llangollen Museum')]: { score: 78, sourceUrl: 'https://llangollenmuseum.org.uk/' },
  },
  Ruthin: {
    [normalise('Ruthin Gaol Museum')]: { score: 87, sourceUrl: 'https://www.denbighshire.gov.uk/en/leisure-and-tourism/museums-and-historic-houses/ruthin-gaol.aspx' },
    [normalise('Nantclwyd House')]: { score: 84, sourceUrl: 'https://www.denbighshire.gov.uk/en/leisure-and-tourism/museums-and-historic-houses/nantclwyd-y-dre.aspx' },
  },
  Denbigh: {
    [normalise('Denbigh Castle')]: { score: 87, sourceUrl: 'https://cadw.gov.wales/visit/places-to-visit/denbigh-castle-and-town-walls' },
    [normalise('Denbigh Museum')]: { score: 78, sourceUrl: 'https://www.denbighmuseum.co.uk/' },
    [normalise('Wireless in Wales')]: { score: 76, sourceUrl: 'https://wirelessinwalesmuseum.co.uk/' },
  },
  Holywell: {
    [normalise("St Winefride's Well")]: { score: 88, sourceUrl: 'https://www.stwinefrideswell.com/' },
    [normalise('St Winefride’s Well Museum')]: { score: 78, sourceUrl: 'https://www.stwinefrideswell.com/' },
  },
  'Colwyn Bay': {
    [normalise('Welsh Mountain Zoo')]: { score: 88, sourceUrl: 'https://www.welshmountainzoo.org/' },
  },
  Rhuddlan: {
    [normalise('Rhuddlan Castle')]: { score: 87, sourceUrl: 'https://cadw.gov.wales/visit/places-to-visit/rhuddlan-castle' },
  },
  Flint: {
    [normalise('Castell y Fflint')]: { score: 84, sourceUrl: 'https://cadw.gov.wales/visit/places-to-visit/flint-castle' },
    [normalise('Flint Castle')]: { score: 84, sourceUrl: 'https://cadw.gov.wales/visit/places-to-visit/flint-castle' },
  },
  Wrexham: {
    [normalise('Xplore! Science Discovery Centre')]: { score: 84, sourceUrl: 'https://www.xplorescience.co.uk/' },
    [normalise('Xplore!')]: { score: 84, sourceUrl: 'https://www.xplorescience.co.uk/' },
    [normalise('Parish Church of St Giles')]: { score: 80, sourceUrl: 'https://www.stgilesparishchurchwrexham.org.uk/' },
    [normalise('Wrexham Museum')]: { score: 78, sourceUrl: 'https://www.wrexham.gov.uk/service/wrexham-museum' },
    [normalise('Ty Pawb')]: { score: 76, sourceUrl: 'https://www.typawb.wales/' },
    [normalise('Tŷ Pawb')]: { score: 76, sourceUrl: 'https://www.typawb.wales/' },
  },
  'St Asaph': {
    [normalise('Cathedral Church of St. Asaph')]: { score: 82, sourceUrl: 'https://stasaphcathedral.wales/' },
    [normalise("The Old Bishop's Palace")]: { score: 74, sourceUrl: 'https://www.churchinwales.org.uk/en/structure/st-asaph/' },
  },
  Mold: {
    [normalise('Mold Castle; The Bailey Hill; Castell yr Wyddgrug; Bryn y Beili')]: { score: 76, sourceUrl: 'https://www.flintshire.gov.uk/en/Resident/Leisure-and-Events/Bailey-Hill.aspx' },
  },
};
...
const researchedSupplementalAttractions: Record<string, OsmElement[]> = {
  Conwy: [{
    type: 'way',
    id: 150098557,
    center: { lat: 53.2812181, lon: -3.8269761 },
    tags: {
      name: 'Conwy Town Walls',
      historic: 'citywalls',
      tourism: 'attraction',
      fee: 'no',
      website: 'https://cadw.gov.wales/visit/places-to-visit/conwy-town-walls',
    },
  }],
  Llandudno: [{
    type: 'way',
    id: 45855764,
    center: { lat: 53.3300699, lon: -3.8270888 },
    tags: {
      name: 'Llandudno Pier',
      man_made: 'pier',
      tourism: 'attraction',
      fee: 'no',
      website: 'https://www.visitconwy.org.uk/listing/llandudno-pier/27751101/',
    },
  }, {
    type: 'node',
    id: 298820079,
    ...
    tags: {
      ...
      tourism: 'attraction',
      railway: 'tram_stop',
      website: 'https://greatormetramway.co.uk/',
    },
  }],
  Llanrwst: [{
    type: 'way',
    id: 156359315,
    center: { lat: 53.1369718, lon: -3.7976358 },
    tags: {
      name: 'Pont Fawr',
      historic: 'bridge',
      tourism: 'attraction',
      fee: 'no',
      website: 'https://www.visitconwy.org.uk/explore/towns-and-villages/llanrwst/',
    },
  }],
  Llangollen: [{
    type: 'node',
    ...
    tags: {
      ...
      tourism: 'attraction',
      railway: 'station',
      website: 'https://llangollen-railway.co.uk/',
    },
  }],
};

const excludedAttractionNames: Record<string, Set<string>> = {
  Ruthin: new Set([normalise('Ruthin Castle'), normalise('Ruthin Castle, Ruthin')]),
};

const visitorBoundaryExtensionNames: Record<string, Set<string>> = {
  Llandudno: new Set([
    normalise('Great Orme Visitor Centre'),
    normalise('Great Orme Tramway'),
    normalise('Great Orme Bronze Age Mines'),
    normalise('Llandudno Pier'),
  ]),
  Llanrwst: new Set([
    normalise('Pont Fawr'),
    normalise("Tu Hwnt i'r Bont"),
  ]),
  Rhuddlan: new Set([normalise('Rhuddlan Castle')]),
  Greenfield: new Set([
    normalise('Abbey Farm Museum'),
    normalise('Greenfield Valley Museum'),
    normalise('Greenfield Valley Rail and Steam Museum'),
    normalise('Lower Cotton Mill'),
  ]),
};
...
async function buildTown(
  ...
  const projectId = `${slug}-clwyd-wales`;
  ...
  const packageData: ProjectPackage = {
    project: {
      ...
      townGuide: {
        ...
        perfectFor: rating === 0 ? ['Visitors already nearby'] : ['A short local detour', 'Visitors exploring Clwyd'],
        suggestedFirstVisit: {
...
async function writeGeneratedModule(projectIds: string[]) {
  ...
  const contents = `// Generated by scripts/create-clwyd-settlement-batch.ts. Do not edit by hand.\nimport type { ProjectPackage } from '../domain/models';\n${imports}\n\nexport const clwydSettlementPackages: ProjectPackage[] = [\n${items}\n];\n`;
...
await writeFile(resolve(reviewDirectory, `clwyd-settlement-batch-audit-${reviewedDate}.json`), `${JSON.stringify(report, null, 2)}\n`, 'utf8');
