...
const manifestPath = resolve('data/imports/northamptonshire-settlements-2026-08-09.json');
const projectsDirectory = resolve('data/projects');
const reviewDirectory = resolve('data/review');
const cacheDirectory = resolve('tmp/northamptonshire-settlement-batch-v2');
...
const generatedModulePath = resolve('src/data/northamptonshireSettlements.generated.ts');
...
const regionalExtent = { south: 51.85, west: -1.4, north: 52.75, east: -0.2 };
...
async function fetchNominatimBoundary(locality: string) {
  ...
  try {
    ...
    const query = new URLSearchParams({
      q: `${locality}, Northamptonshire, England`,
      format: 'jsonv2',
      limit: '8',
      countrycodes: 'gb',
      addressdetails: '1',
  ...
  const result = results
    .map((candidate) => ({
      ...
      score: (normalise(candidate.address?.village ?? candidate.address?.town ?? candidate.address?.hamlet ?? candidate.address?.suburb ?? '') === requested ? 5 : 0)
        + (/northamptonshire/i.test(candidate.display_name) ? 3 : 0)
...
async function fetchBroadGreenSpaces() {
  const responses = await Promise.all(broadBboxes.map(async ([south, west, north, east], index) => {
    ...
    return fetchOverpass(query, `green-spaces-${index}.json`);
  }));
  const byId = new Map<string, OsmElement>();
  for (const response of responses) for (const element of response.elements ?? []) byId.set(`${element.type}-${element.id}`, element);
  return [...byId.values()];
...
function treasureMatch(locality: string, products: TreasureProduct[]) {
  ...
  const allowedLocations = new Set(
    (['Northamptonshire', 'North Northamptonshire', 'West Northamptonshire', regionFor(locality)])
      .map(normalise),
  );
...
function regionFor(locality: string) {
  const edges: Record<string, string> = {
    ...
    Braybrooke: 'Leicestershire',
    Cottingham: 'North Northamptonshire',
    'Easton on the Hill': 'North Northamptonshire',
  };
  return edges[locality] ?? 'Northamptonshire';
}
...
async function buildTown(
  ...
  for (const element of elements.filter(attractiveOsmPlace)) {
    ...
    const score = attractionScore(tags);
  ...
  for (const feature of features.filter((candidate) => candidate.tags.includes('nhle'))) {
    const score = nhleAttractionScore(feature);
  ...
  const packageData: ProjectPackage = {
    project: {
      ...
      townGuide: {
        ...
        perfectFor: rating === 0 ? ['Visitors already nearby'] : ['A short local detour', 'Visitors exploring Northamptonshire'],
        suggestedFirstVisit: {
...
async function writeGeneratedModule(projectIds: string[]) {
  ...
  const contents = `// Generated by scripts/create-northamptonshire-settlement-batch.ts. Do not edit by hand.\nimport type { ProjectPackage } from '../domain/models';\n${imports}\n\nexport const northamptonshireSettlementPackages: ProjectPackage[] = [\n${items}\n];\n`;
...
for (const record of onsRecords) onsByName.set(normalise(record.BUA24NM), [...(onsByName.get(normalise(record.BUA24NM)) ?? []), record]);
...
await writeFile(resolve(reviewDirectory, `northamptonshire-settlement-batch-audit-${reviewedDate}.json`), `${JSON.stringify(report, null, 2)}\n`, 'utf8');
