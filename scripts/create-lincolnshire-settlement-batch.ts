...
const manifestPath = resolve('data/imports/lincolnshire-settlements-2026-08-10.json');
const projectsDirectory = resolve('data/projects');
const reviewDirectory = resolve('data/review');
const cacheDirectory = resolve('tmp/lincolnshire-settlement-batch-v1');
...
const generatedModulePath = resolve('src/data/lincolnshireSettlements.generated.ts');
...
const duplicateLocalities = new Set([
  'broughton',
  'burwell',
  'doddington',
  'holme',
...
const regionalExtent = { south: 52.55, west: -1.0, north: 53.9, east: 0.4 };
...
async function fetchNominatimBoundary(locality: string) {
  ...
  try {
    ...
    const query = new URLSearchParams({
      q: `${locality}, Lincolnshire, England`,
      format: 'jsonv2',
      limit: '8',
      countrycodes: 'gb',
      addressdetails: '1',
  ...
  const result = results
    .map((candidate) => ({
      ...
      score: (normalise(candidate.address?.village ?? candidate.address?.town ?? candidate.address?.hamlet ?? candidate.address?.suburb ?? '') === requested ? 5 : 0)
        + (/(?:lincolnshire|north lincolnshire|north east lincolnshire)/i.test(candidate.display_name) ? 3 : 0)
...
function treasureMatch(locality: string, products: TreasureProduct[]) {
  const requested = normalise(locality);
  const allowedLocations = new Set(['Lincolnshire', 'North Lincolnshire', 'North East Lincolnshire'].map(normalise));
...
const northLincolnshireLocalities = new Set([
  'alkborough', 'appleby', 'barrow upon humber', 'barton upon humber', 'brigg',
  'broughton', 'burton upon stather', 'elsham', 'haxey', 'keadby',
  'kirton in lindsey', 'messingham', 'normanby', 'owston ferry', 'roxby',
  'scawby', 'scunthorpe', 'south ferriby', 'thornton curtis', 'west halton',
  'whitton', 'winteringham', 'winterton', 'wrawby',
]);
const northEastLincolnshireLocalities = new Set([
  'bradley', 'cleethorpes', 'grimsby', 'hatcliffe', 'humberston', 'immingham',
  'laceby', 'waltham', 'wold newton',
]);

function regionFor(locality: string) {
  const key = normalise(locality);
  if (northEastLincolnshireLocalities.has(key)) return 'North East Lincolnshire';
  if (northLincolnshireLocalities.has(key)) return 'North Lincolnshire';
  return 'Lincolnshire';
}
...
async function buildTown(
  ...
  const projectId = duplicateLocalities.has(normalise(locality))
    ? `${slug}-lincolnshire-england`
    : `${slug}-england`;
  ...
  const packageData: ProjectPackage = {
    project: {
      ...
      townGuide: {
        ...
        perfectFor: rating === 0 ? ['Visitors already nearby'] : ['A short local detour', 'Visitors exploring Lincolnshire'],
        suggestedFirstVisit: {
...
async function writeGeneratedModule(projectIds: string[]) {
  ...
  const contents = `// Generated by scripts/create-lincolnshire-settlement-batch.ts. Do not edit by hand.\nimport type { ProjectPackage } from '../domain/models';\n${imports}\n\nexport const lincolnshireSettlementPackages: ProjectPackage[] = [\n${items}\n];\n`;
...
await writeFile(resolve(reviewDirectory, `lincolnshire-settlement-batch-audit-${reviewedDate}.json`), `${JSON.stringify(report, null, 2)}\n`, 'utf8');
