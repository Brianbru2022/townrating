...
const reviewedAt = '2026-09-02T18:30:00.000Z';
const stems = [
  'glenduckie',
  'luthrie',
  'moonzie',
...
const listed = JSON.parse(
  await readFile(
    resolve('data/reference/scotland-hes/hes-listed-building-descriptions.json'),
    'utf8',
  ),
) as Record<string, CachedDescription>;
const scheduled = JSON.parse(
  await readFile(
    resolve('data/reference/scotland-hes/hes-scheduled-monument-descriptions.json'),
    'utf8',
  ),
) as Record<string, CachedDescription>;
...
function evidence(feature: HeritageFeature): {
  ...
  const text = [
    ...
    feature.name,
    ...feature.sourceRecords.map((record) => record.notes),
  ]
    .filter(Boolean)
    .join('. ');
  return {
...
function applyDate(
  ...
  const dateSource: SourceRecord = {
    ...
    reliability:
      feature.tags.includes('hes-scheduled-monument') ||
      feature.tags.includes('hes-listed-building')
        ? 'official_statutory'
        : 'official_non_statutory',
    notes:
      'Material historic date normalised from the local HES description/classification. Administrative designation dates are excluded.',
  };
...
for (const stem of stems) {
  ...
  projects.push({
    ...
    totalHesAndNrhe: features.length,
    insideBoundary: inside.length,
    listedBuildings: features.filter((feature) => feature.tags.includes('hes-listed-building'))
      .length,
    scheduledMonuments: features.filter((feature) =>
      feature.tags.includes('hes-scheduled-monument'),
    ).length,
    nrhe: features.filter(
      (feature) => feature.tags.includes('nrhe') || feature.tags.includes('hes-nrhe'),
    ).length,
    enriched,
    retainedExisting,
    datedInside: inside.filter(hasCompleteDate).length,
    undatedInsideRetainedHidden: inside.filter(
      (feature) => !hasCompleteDate(feature) && feature.tags.includes('map-hidden'),
    ).length,
    visibleHeritagePins: visible.length,
    visiblePinsWithoutDates: visible.filter((feature) => !hasCompleteDate(feature)).length,
    visiblePinNamesContainingAppendedDate: visible.filter((feature) =>
...
await writeFile(
  resolve('data/review/glenduckie-ladybank-hes-date-certification-2026-09-02.json'),
  `${JSON.stringify({ reviewedAt, sourceMode: 'local HES Listed Buildings, Scheduled Monuments and NRHE datasets with locally cached official descriptions', rule: 'All HES/NRHE records remain intact. Only records with defensible material dates are map-visible. Dates are data fields, never appended to map labels.', projects, totals: { records: projects.reduce((sum, item) => sum + Number(item.totalHesAndNrhe), 0), visiblePins: projects.reduce((sum, item) => sum + Number(item.visibleHeritagePins), 0), visiblePinsWithoutDates: projects.reduce((sum, item) => sum + Number(item.visiblePinsWithoutDates), 0), visiblePinNamesContainingAppendedDate: projects.reduce((sum, item) => sum + Number(item.visiblePinNamesContainingAppendedDate), 0) } }, null, 2)}\n`,
  'utf8',
);
