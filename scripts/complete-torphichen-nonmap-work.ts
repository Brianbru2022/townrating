...
const completionReviewPath = resolve(process.argv[3] ?? 'data/review/torphichen-completion-review.json');
const linkedAuditPath = resolve('data/review/torphichen-linked-source-audit.json');
const currentPlaceCurationPath = resolve('data/curation/torphichen-current-place-curation.json');
...
const statutoryEvidence = new Map<string, DateEvidence>([
  [
    'hes-scheduled-monument:SM90305',
    {
      text: 'Torphichen Preceptory first founded in the 12th century, with major additions in the 14th and 15th centuries',
      earliest: 1100,
      latest: 1199,
      precision: 'HES scheduled-monument foundation century',
      confidence: 'medium',
      sourceName: 'Historic Environment Scotland scheduled monument designation',
      sourceRecordId: 'SM90305',
      sourceUrl: 'https://portal.historicenvironment.scot/designation/SM90305',
      reliability: 'official_statutory',
      notes:
        'HES description states the Preceptory was first founded in the 12th century and that the remains are of 12th-century foundation with major additions in the 14th and 15th centuries.',
    },
  ],
  [
    'hes-scheduled-monument:SM1903',
    {
      text: 'Refuge stone associated with the 12th-century Torphichen Preceptory sanctuary',
      earliest: 1100,
      latest: 1199,
      precision: 'associated medieval sanctuary century',
      confidence: 'low',
      sourceName: 'Historic Environment Scotland scheduled monument designation',
      sourceRecordId: 'SM1903',
      sourceUrl: 'https://portal.historicenvironment.scot/designation/SM1903',
      reliability: 'official_statutory',
      notes:
        'HES description identifies the refuge stone as marking the centre of the sanctuary associated with Torphichen Preceptory, founded by the Knights Hospitallers in the 12th century. The date is contextual rather than a precise carving date.',
    },
  ],
]);
...
function applyDate(feature: HeritageFeature, evidence: DateEvidence): void {
  ...
  addTags(feature, 'date-reviewed', 'curation-date-enriched', 'torphichen-nonmap-date-reviewed');
  appendReviewNote(
    feature,
    'Torphichen completion review: broad date evidence has been normalised from the linked official HES/NRHE record.',
  );
}
...
function markNoDate(feature: HeritageFeature): void {
  feature.reviewed = true;
  feature.updatedAt = reviewedAt;
  addTags(feature, 'date-reviewed', 'reviewed-no-defensible-date', 'torphichen-date-reviewed-no-date');
  appendReviewNote(
    feature,
    'Torphichen completion review: the official source was checked and no defensible construction or historic-period date is published for this record.',
  );
}
...
function currentSummary(feature: HeritageFeature): string {
  ...
  if (generic.has(feature.name)) return `Current ${label} in Torphichen recorded by OpenStreetMap.`;
  return `${feature.name} is a current ${label} in Torphichen recorded by OpenStreetMap.`;
}
...
const linkedSourceAudit = pkg.features
  ...
  .map(({ feature, kinds }) => {
    feature.reviewed = true;
    feature.updatedAt = reviewedAt;
    addTags(feature, 'torphichen-linked-source-reviewed');
    appendReviewNote(
      feature,
  ...
  `${JSON.stringify(
    {
      ...
      reviewedAt,
      policy:
        'Completed Torphichen non-map curation. Explicit HES/NRHE period evidence was normalised conservatively; period-unknown, period-unassigned, event and conservation-area records were marked reviewed without inventing dates.',
      dated,
      reviewedWithoutDate,
      currentPlaceReview: {
...
console.log(
  `Completed Torphichen non-map work: ${dated.length} date review(s), ${reviewedWithoutDate.length} reviewed-without-date record(s), ${currentPlaceEntries.length} current place(s), ${currentParksReviewed} current park(s), ${linkedSourceAudit.length} linked-source audit record(s).`,
);
