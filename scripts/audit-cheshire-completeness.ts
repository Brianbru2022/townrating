...
const batchAuditPath = resolve('data/review/cheshire-settlement-batch-audit-2026-08-12.json');
const dateAuditPath = resolve('data/review/cheshire-nhle-date-enrichment-2026-08-12.json');
const jsonOutputPath = resolve(`data/review/cheshire-full-completeness-audit-${reviewedDate}.json`);
const markdownOutputPath = resolve(`data/review/cheshire-full-completeness-audit-${reviewedDate}.md`);
...
async function main() {
  ...
  for (const { package: projectPackage } of projects) {
    ...
    if (!projectPackage.project.id.endsWith('-cheshire-england')) {
  ...
  const report = {
    ...
    scope: `All ${manifest.settlements.length} OSM city, town and village records in Cheshire East, Cheshire West and Chester, Halton and Warrington.`,
    methodology: [
  ...
  const markdown = `# Cheshire town completeness and compliance audit\n\n`
  ...
  console.log(`Cheshire audit: ${report.compliance.hardRulesPassed ? 'PASS' : 'FAIL'}; ${projects.length} towns; ${hardFailureCount} hard failure(s).`);
