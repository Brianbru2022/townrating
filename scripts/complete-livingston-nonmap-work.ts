...
const completionReviewPath = resolve(process.argv[3] ?? 'data/review/livingston-completion-review.json');
const linkedAuditPath = resolve('data/review/livingston-linked-source-audit.json');
const currentPlaceCurationPath = resolve('data/curation/livingston-current-place-curation.json');
...
function applyDate(feature: HeritageFeature, evidence: DateEvidence): void {
  ...
  addTags(feature, 'date-reviewed', 'curation-date-enriched', 'livingston-nonmap-date-reviewed');
  appendReviewNote(
    feature,
    'Livingston completion review: broad date evidence has been normalised from the linked official HES/NRHE record.',
  );
}
...
function markNoDate(feature: HeritageFeature): void {
  ...
  appendReviewNote(
    feature,
    'Livingston completion review: the official source was checked and no defensible construction or historic-period date is published for this record.',
  );
}
...
function currentSummary(feature: HeritageFeature): string {
  ...
  if (generic.has(feature.name)) return `Current ${label} in Livingston recorded by OpenStreetMap.`;
  return `${feature.name} is a current ${label} in Livingston recorded by OpenStreetMap.`;
}
...
const linkedSourceAudit = pkg.features
  ...
  .map(({ feature, kinds }) => {
    feature.reviewed = true;
    feature.updatedAt = reviewedAt;
    addTags(feature, 'livingston-linked-source-reviewed');
    appendReviewNote(
      feature,
  ...
  `${JSON.stringify(
    {
      ...
      reviewedAt,
      policy:
        'Completed Livingston non-map curation, including Livingston Village. Explicit HES/NRHE period evidence was normalised conservatively; period-unknown, period-unassigned, event, general-view and conservation-area records were marked reviewed without inventing dates.',
      dated,
      reviewedWithoutDate,
      currentPlaceReview: {
...
console.log(
  `Completed Livingston non-map work: ${dated.length} date review(s), ${reviewedWithoutDate.length} reviewed-without-date record(s), ${currentPlaceEntries.length} current place(s), ${currentParksReviewed} current park(s), ${linkedSourceAudit.length} linked-source audit record(s).`,
);
