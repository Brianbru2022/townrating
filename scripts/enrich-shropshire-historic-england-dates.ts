...
const cachePath = resolve(
  process.argv[2] ?? `data/review/shropshire-nhle-official-text-${auditDate}.json`,
);
const auditPath = resolve(
  process.argv[3] ?? `data/review/shropshire-nhle-date-enrichment-${auditDate}.json`,
);
...
async function projectPaths() {
  ...
  if (!paths.length) throw new Error('No Shropshire project imports were found.');
  return paths;
}
...
console.log(
  `Enriched ${enriched.length}/${totalNhle} Shropshire NHLE record(s); ` +
