...
const cachePath = resolve(
  process.argv[2] ?? `data/review/leicestershire-nhle-official-text-${auditDate}.json`,
);
const auditPath = resolve(
  process.argv[3] ?? `data/review/leicestershire-nhle-date-enrichment-${auditDate}.json`,
);
...
async function projectPaths() {
  ...
  if (!paths.length) throw new Error('No Leicestershire project imports were found.');
  return paths;
}
...
const queue = [...references.values()].filter(
...
for (const { path, pkg } of packages) {
  ...
  pkg.project.researchNotes =
    `${baseResearchNotes} Historic England official list-entry date enrichment completed ${auditDate}: ` +
  ...
  `${JSON.stringify(
    {
      ...
      cachePath,
      methodology:
        'Normalised the earliest dated fabric or historic component from cached official Historic England list-entry text. Administrative listing dates and restoration-only dates were excluded.',
      counts: {
...
console.log(
  `Enriched ${enriched.length}/${totalNhle} Leicestershire NHLE record(s); ` +
