...
const manifestPath = resolve('data/imports/leicestershire-settlements-2026-08-11.json');
const projectsDirectory = resolve('data/projects');
const reviewDirectory = resolve('data/review');
const cacheDirectory = resolve('tmp/leicestershire-settlement-batch-v1');
...
const generatedModulePath = resolve('src/data/leicestershireSettlements.generated.ts');
...
const regionalExtent = { south: 52.35, west: -1.8, north: 53.05, east: -0.6 };
...
async function fetchNominatimBoundary(locality: string) {
  ...
  try {
    ...
    const query = new URLSearchParams({
      q: `${locality}, Leicestershire, England`,
      format: 'jsonv2',
      limit: '8',
      countrycodes: 'gb',
      addressdetails: '1',
  ...
  const result = results
    .map((candidate) => ({
      ...
      score: (normalise(candidate.address?.village ?? candidate.address?.town ?? candidate.address?.hamlet ?? candidate.address?.suburb ?? '') === requested ? 5 : 0)
        + (/(?:leicestershire|city of leicester)/i.test(candidate.display_name) ? 3 : 0)
...
async function resolveBoundary(
  ...
  const regionalBoundaries = exactBoundaries.filter(({ boundary }) => {
  ...
  let osmHint: Awaited<ReturnType<typeof fetchNominatimBoundary>> | undefined;
  try {
    osmHint = await fetchNominatimBoundary(locality);
  } catch (error) {
    if (regionalBoundaries.length === 0) throw error;
  }
  const hint = osmHint ? pointOnFeature(osmHint.originalBoundary).geometry.coordinates : undefined;
  const rankedBoundaries = regionalBoundaries
    .map((candidate) => {
      const [longitude, latitude] = pointOnFeature(candidate.boundary).geometry.coordinates;
      if (!hint) return { candidate, metres: 0 };
      const latitudeMetres = (latitude - hint[1]) * 111_320;
      const longitudeMetres = (longitude - hint[0]) * 111_320 * Math.cos(latitude * Math.PI / 180);
      return { candidate, metres: Math.hypot(latitudeMetres, longitudeMetres) };
    })
    .sort((left, right) => left.metres - right.metres);
  const exact = rankedBoundaries[0] && rankedBoundaries[0].metres <= 15_000
    ? rankedBoundaries[0].candidate
    : undefined;
  if (exact) {
  ...
  const osm = osmHint ?? await fetchNominatimBoundary(locality);
...
function treasureMatch(locality: string, products: TreasureProduct[]) {
  const requested = normalise(locality);
  const allowedLocations = new Set(['Leicestershire', 'Leicester'].map(normalise));
...
const researchedAttractionScores: Record<string, Record<string, number>> = {
  Leicester: {
    'national space centre': 93,
    'king richard iii visitor centre': 90,
    'leicester cathedral': 88,
    'leicester museum and art gallery': 88,
    'new walk museum and art gallery': 88,
    'abbey pumping station': 86,
    'jewry wall museum': 85,
    'the guildhall': 84,
  },
  'Ashby-de-la-Zouch': {
    'ashby de la zouch castle': 86,
    'ashby de la zouch museum': 78,
  },
  Loughborough: {
    'great central railway': 90,
    'john taylor bellfoundry museum': 85,
    'charnwood museum': 78,
  },
  'Melton Mowbray': {
    'melton carnegie museum': 80,
  },
  'Market Harborough': {
    'harborough museum': 78,
  },
};
...
function regionFor(locality: string) {
  return normalise(locality) === 'leicester' ? 'City of Leicester' : 'Leicestershire';
}
...
async function buildTown(
  ...
  const projectId = `${slug}-leicestershire-england`;
  ...
  const packageData: ProjectPackage = {
    project: {
      ...
      townGuide: {
        ...
        perfectFor: rating === 0 ? ['Visitors already nearby'] : ['A short local detour', 'Visitors exploring Leicestershire'],
        suggestedFirstVisit: {
...
async function writeGeneratedModule(projectIds: string[]) {
  ...
  const contents = `// Generated by scripts/create-leicestershire-settlement-batch.ts. Do not edit by hand.\nimport type { ProjectPackage } from '../domain/models';\n${imports}\n\nexport const leicestershireSettlementPackages: ProjectPackage[] = [\n${items}\n];\n`;
...
await writeFile(resolve(reviewDirectory, `leicestershire-settlement-batch-audit-${reviewedDate}.json`), `${JSON.stringify(report, null, 2)}\n`, 'utf8');
