...
const completionReviewPath = resolve(
  process.argv[3] ?? 'data/review/broxburn-and-uphall-completion-review.json',
);
const linkedAuditPath = resolve('data/review/broxburn-and-uphall-linked-source-audit.json');
const currentPlaceCurationPath = resolve('data/curation/broxburn-and-uphall-current-place-curation.json');
...
const statutoryEvidence = new Map<string, DateEvidence>([
  [
    'hes-designed-landscape:GDL00298',
    {
      text: 'Newliston designed landscape dating back to William Adam plan made in 1725',
      earliest: 1725,
      latest: 1725,
      precision: 'documented designed-landscape plan year',
      confidence: 'high',
      sourceName: 'Historic Environment Scotland garden and designed landscape designation',
      sourceRecordId: 'GDL00298',
      sourceUrl: 'https://portal.historicenvironment.scot/designation/GDL00298',
      reliability: 'official_statutory',
      notes:
        'HES Inventory description states the designed landscape dates back to a William Adam plan made in 1725.',
    },
  ],
  [
    'hes-scheduled-monument:SM8954',
    {
      text: 'Union Canal designed by Hugh Baird and built 1818-23',
      earliest: 1818,
      latest: 1823,
      precision: 'documented canal construction range',
      confidence: 'high',
      sourceName: 'Historic Environment Scotland scheduled monument designation',
      sourceRecordId: 'SM8954',
      sourceUrl: 'https://portal.historicenvironment.scot/designation/SM8954',
      reliability: 'official_statutory',
      notes:
        'HES statement of national importance describes the Union Canal as designed by Hugh Baird and built 1818-23.',
    },
  ],
]);
...
function evidenceFromUnionCanalContext(feature: HeritageFeature): DateEvidence | undefined {
  if (!feature.id.startsWith('nrhe:') || !/^UNION CANAL\b/i.test(feature.name)) return undefined;
  return {
    text: 'Union Canal designed by Hugh Baird and built 1818-23',
    earliest: 1818,
    latest: 1823,
    precision: 'documented canal construction range',
    confidence: 'medium',
    sourceName: 'Historic Environment Scotland scheduled monument contextual date review',
    sourceRecordId: 'SM8954',
    sourceUrl: 'https://portal.historicenvironment.scot/designation/SM8954',
    reliability: 'official_statutory',
    notes:
      'Applied to an NRHE Union Canal component because the HES scheduled-monument record dates the canal and listed structures to the 1818-23 construction campaign.',
  };
}
...
function applyDate(feature: HeritageFeature, evidence: DateEvidence): void {
  ...
  addTags(feature, 'date-reviewed', 'curation-date-enriched', 'broxburn-uphall-nonmap-date-reviewed');
  appendReviewNote(
    feature,
    'Broxburn and Uphall completion review: broad date evidence has been normalised from the linked official HES/NRHE record.',
  );
}
...
function markNoDate(feature: HeritageFeature): void {
  feature.reviewed = true;
  feature.updatedAt = reviewedAt;
  addTags(feature, 'date-reviewed', 'reviewed-no-defensible-date', 'broxburn-uphall-date-reviewed-no-date');
  appendReviewNote(
    feature,
    'Broxburn and Uphall completion review: the official source was checked and no defensible construction or historic-period date is published for this record.',
  );
}
...
function currentSummary(feature: HeritageFeature): string {
  ...
  if (generic.has(feature.name)) return `Current ${label} in Broxburn and Uphall recorded by OpenStreetMap.`;
  return `${feature.name} is a current ${label} in Broxburn and Uphall recorded by OpenStreetMap.`;
}
...
for (const feature of pkg.features) {
  if (!feature.documentedDateText && (feature.id.startsWith('nrhe:') || feature.id.startsWith('hes-'))) {
    const evidence =
      statutoryEvidence.get(feature.id) ?? evidenceFromUnionCanalContext(feature) ?? evidenceFromClassification(feature);
    if (evidence) {
...
const linkedSourceAudit = pkg.features
  .map((feature) => {
    ...
    for (const source of feature.sourceRecords) {
      ...
      if (/listed building|designation|scheduled monument|garden and designed landscape|Designations GIS/i.test(source.sourceName))
  ...
  .map(({ feature, kinds }) => {
    feature.reviewed = true;
    feature.updatedAt = reviewedAt;
    addTags(feature, 'broxburn-uphall-linked-source-reviewed');
    appendReviewNote(
      feature,
  ...
  `${JSON.stringify(
    {
      ...
      reviewedAt,
      policy:
        'Completed Broxburn and Uphall non-map curation. Explicit HES/NRHE period evidence was normalised conservatively; period-unknown, period-unassigned, event, general-view and conservation-area records were marked reviewed without inventing dates, except Union Canal components dated from the official HES scheduled-monument statement.',
      dated,
      reviewedWithoutDate,
      currentPlaceReview: {
...
console.log(
  `Completed Broxburn and Uphall non-map work: ${dated.length} date review(s), ${reviewedWithoutDate.length} reviewed-without-date record(s), ${currentPlaceEntries.length} current place(s), ${currentParksReviewed} current park(s), ${linkedSourceAudit.length} linked-source audit record(s).`,
);
