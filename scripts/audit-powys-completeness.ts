...
const batchAuditPath = resolve(`data/review/powys-settlement-batch-audit-${reviewedDate}.json`);
const jsonOutputPath = resolve(`data/review/powys-full-completeness-audit-${reviewedDate}.json`);
const markdownOutputPath = resolve(`data/review/powys-full-completeness-audit-${reviewedDate}.md`);
...
async function main() {
  ...
  const boundaryContainmentFailures: Array<{ projectId: string; issues: string[] }> = [];
  const inventoryFailures: Array<{ locality?: string; issues: string[] }> = [];
  const boundaryDerivedCentreDifferences: string[] = [];
  const mapCentresInVisitorExtensions: string[] = [];
  const osmCentresOutsidePreservedBoundary: string[] = [];

  if (!manifest.inventory || manifest.inventory.length !== manifest.settlements.length) {
    inventoryFailures.push({ issues: ['manifest inventory is missing or does not match the settlement list'] });
  }
  const inventoryByLocality = new Map((manifest.inventory ?? []).map((entry) => [entry.locality, entry]));
  ...
  for (const { package: projectPackage } of projects) {
    ...
    const containmentIssues: string[] = [];
    const centrePoint = point(projectPackage.project.centre);
    if (boundary?.geometry && !booleanPointInPolygon(centrePoint, boundary)) {
      containmentIssues.push('town centre is outside the active visitor boundary');
    }
    const originalBoundary = projectPackage.project.townStudyArea?.localityBoundary;
    if (originalBoundary?.geometry && !booleanPointInPolygon(centrePoint, originalBoundary)) {
      mapCentresInVisitorExtensions.push(projectPackage.project.locality);
    }
    const inventoryEntry = inventoryByLocality.get(projectPackage.project.locality);
    if (!inventoryEntry) {
      inventoryFailures.push({ locality: projectPackage.project.locality, issues: ['project has no manifest inventory entry'] });
    } else {
      const inventoryIssues: string[] = [];
      if (!['city', 'town', 'village'].includes(inventoryEntry.placeType)) inventoryIssues.push('invalid settlement place type');
      if (!inventoryEntry.sourceUrl) inventoryIssues.push('missing source URL');
      if (Math.abs(inventoryEntry.centre[0] - projectPackage.project.centre[0]) > 1e-7
        || Math.abs(inventoryEntry.centre[1] - projectPackage.project.centre[1]) > 1e-7) {
        boundaryDerivedCentreDifferences.push(projectPackage.project.locality);
      }
      const inventoryPoint = point(inventoryEntry.centre);
      if (boundary?.geometry && !booleanPointInPolygon(inventoryPoint, boundary)) {
        containmentIssues.push('audited OSM settlement point is outside the active visitor boundary');
      }
      if (originalBoundary?.geometry && !booleanPointInPolygon(inventoryPoint, originalBoundary)) {
        osmCentresOutsidePreservedBoundary.push(projectPackage.project.locality);
      }
      if (inventoryIssues.length > 0) inventoryFailures.push({ locality: projectPackage.project.locality, issues: inventoryIssues });
    }
    if (containmentIssues.length > 0) boundaryContainmentFailures.push({
    ...
    if (!projectPackage.project.id.endsWith('-powys-wales')) {
      sourceFailures.push({ projectId: projectPackage.project.id, issues: ['project ID is not region-safe'] });
    }
    if (projectPackage.project.region !== 'Powys') {
      sourceFailures.push({ projectId: projectPackage.project.id, issues: ['project region is not Powys'] });
    }
    const [longitude, latitude] = projectPackage.project.centre;
    if (longitude < -3.95 || longitude > -2.95 || latitude < 51.70 || latitude > 52.95) {
  ...
  const hardFailureCount = missingPackages.length + unexpectedPackages.length
    ...
    + sourceFailures.length + boundaryContainmentFailures.length + inventoryFailures.length
  ...
  const report = {
    ...
    scope: `All ${manifest.settlements.length} towns in the bundled Powys settlement manifest.`,
    methodology: [
      ...
      'Checked every audited OSM settlement point and map centre against the active visitor boundary, while separately recording OSM label points that need a documented extension beyond the preserved statistical boundary.',
    ...
    counts: {
      ...
      boundaries: {
        ...
        boundaryDerivedMapCentres: boundaryDerivedCentreDifferences.length,
        mapCentresInVisitorExtensions: mapCentresInVisitorExtensions.length,
        osmCentresOutsidePreservedBoundary: osmCentresOutsidePreservedBoundary.length,
      },
      heatMapDates,
    ...
    auditLimitations: [
      'A full live Overpass refresh was attempted for this dated audit but did not complete within the public service timeout. OSM visitor records were therefore checked against the complete cached source snapshot used by the Powys batch generator.',
  ...
  const markdown = `# Powys town completeness and compliance audit\n\n`
    ...
    + `- Every audited OSM settlement point is contained by its active visitor boundary; ${report.counts.boundaries.osmCentresOutsidePreservedBoundary} OSM label point(s) sit just beyond the preserved statistical boundary and use a documented visitor-boundary envelope.\n`
    + `- ${report.counts.boundaries.boundaryDerivedMapCentres} map centres use a boundary-derived representative point instead of the OSM label coordinate; ${report.counts.boundaries.mapCentresInVisitorExtensions} lie in a documented visitor-boundary extension.\n\n`
    + `## Evidence base\n\n`
    ...
    + `- The live all-town Overpass refresh timed out, so this audit uses the complete cached OSM source snapshot retained by the Powys generator.\n\n`
  ...
  console.log(`Powys audit: ${report.compliance.hardRulesPassed ? 'PASS' : 'FAIL'}; ${projects.length} towns; ${hardFailureCount} hard failure(s).`);
