...
const completionReviewPath = resolve(
  process.argv[3] ?? 'data/review/linlithgow-completion-review.json',
);
const linkedAuditPath = resolve('data/review/linlithgow-linked-source-audit.json');
const currentPlaceCurationPath = resolve('data/curation/linlithgow-current-place-curation.json');
...
const statutoryEvidence = new Map<string, DateEvidence>([
  ...
  [
    'hes-scheduled-monument:SM13099',
    {
      text: 'Linlithgow Palace was built between 1424 and 1624',
      earliest: 1424,
      latest: 1624,
      precision: 'documented palace construction range',
      confidence: 'high',
      sourceName: 'Historic Environment Scotland scheduled monument designation',
      sourceRecordId: 'SM13099',
      sourceUrl: 'https://portal.historicenvironment.scot/designation/SM13099',
      reliability: 'official_statutory',
      notes:
        'HES scheduled-monument description dates the palace construction range; earlier buried archaeology within the scheduled area is not narrowed to a single date.',
    },
  ],
]);
...
function applyDate(feature: HeritageFeature, evidence: DateEvidence): void {
  ...
  addTags(feature, 'date-reviewed', 'curation-date-enriched', 'linlithgow-nonmap-date-reviewed');
  appendReviewNote(
    feature,
    'Linlithgow completion review: broad date evidence has been normalised from the linked official HES/NRHE record.',
  );
}
...
function markNoDate(feature: HeritageFeature): void {
  ...
  appendReviewNote(
    feature,
    'Linlithgow completion review: the official source was checked and no defensible construction or historic-period date is published for this record.',
  );
}
...
function currentSummary(feature: HeritageFeature): string {
  ...
  if (generic.has(feature.name)) return `Current ${label} in Linlithgow recorded by OpenStreetMap.`;
  return `${feature.name} is a current ${label} in Linlithgow recorded by OpenStreetMap.`;
}
...
const linkedSourceAudit = pkg.features
  ...
  .map(({ feature, kinds }) => {
    feature.reviewed = true;
    feature.updatedAt = reviewedAt;
    addTags(feature, 'linlithgow-linked-source-reviewed');
    appendReviewNote(
      feature,
  ...
  `${JSON.stringify(
    {
      ...
      reviewedAt,
      policy:
        'Completed Linlithgow non-map curation. Explicit HES/NRHE period evidence was normalised conservatively; period-unknown, period-unassigned, event, general-view and conservation-area records were marked reviewed without inventing dates.',
      dated,
      reviewedWithoutDate,
      currentPlaceReview: {
...
console.log(
  `Completed Linlithgow non-map work: ${dated.length} date review(s), ${reviewedWithoutDate.length} reviewed-without-date record(s), ${currentPlaceEntries.length} current place(s), ${currentParksReviewed} current park(s), ${linkedSourceAudit.length} linked-source audit record(s).`,
);
