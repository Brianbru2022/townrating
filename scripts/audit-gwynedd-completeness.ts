...
const batchAuditPath = resolve('data/review/gwynedd-settlement-batch-audit-2026-08-11.json');
const jsonOutputPath = resolve(`data/review/gwynedd-full-completeness-audit-${reviewedDate}.json`);
const markdownOutputPath = resolve(`data/review/gwynedd-full-completeness-audit-${reviewedDate}.md`);
...
async function main() {
  ...
  for (const { package: projectPackage } of projects) {
    ...
    if (!projectPackage.project.id.endsWith('-gwynedd-wales')) {
      sourceFailures.push({ projectId: projectPackage.project.id, issues: ['project ID is not region-safe'] });
    }
    if (projectPackage.project.region !== 'Gwynedd') {
      sourceFailures.push({ projectId: projectPackage.project.id, issues: ['project region is not Gwynedd'] });
    }
  ...
  const hardFailureCount = missingPackages.length + unexpectedPackages.length
    ...
    + boundaryFailures.length + sourceFailures.length + hardVisitorIssues.length
    + batchCheckFailures.length + batchAudit.failed;

  const report = {
    schemaVersion: 1,
    reviewedAt: `${reviewedDate}T00:00:00Z`,
    scope: `All ${manifest.settlements.length} towns in the bundled Gwynedd settlement manifest.`,
    methodology: [
    ...
    verificationSources: [
      {
        name: 'Cyngor Gwynedd public toilets',
        url: 'https://www.gwynedd.llyw.cymru/en-gb/leisure-country-parks-paths-and-beaches/toilets/public-toilets',
        use: 'Official facility names, seasonal opening and accessible-toilet indicators.',
      },
      {
        name: 'Cyngor Gwynedd community toilets',
        url: 'https://www.gwynedd.llyw.cymru/en-gb/leisure-country-parks-paths-and-beaches/toilets/community-toilets',
        use: 'Public-access business facilities reviewed separately from dedicated public toilets.',
      },
      {
        name: 'Cyngor Gwynedd car parks',
        url: 'https://www.gwynedd.llyw.cymru/en-gb/parking-roads-and-travel/parking-and-permits/car-parks',
        use: 'Council payment policy and ownership research reference.',
      },
      {
        name: 'Eryri National Park walks',
        url: 'https://www.eryri.llyw.cymru/visit/walks/?view=list',
        use: 'Official route-research reference; mapped paths are not promoted as trails without visitor evidence.',
      },
    ],
    auditLimitations: [
      'A full live Overpass refresh was attempted for this dated audit but did not complete within the public service timeout. OSM visitor records were therefore checked against the complete cached source snapshot used by the Gwynedd batch generator.',
  ...
  const markdown = `# Gwynedd town completeness and compliance audit\n\n`
    ...
    + `- Dedicated public-toilet names and seasonal/accessibility notes were checked against the Cyngor Gwynedd register.\n`
    + `- The community-toilet register was reviewed separately; participating businesses are not silently promoted as dedicated public toilets without a matched location.\n`
    + `- The Council car-park directory confirms its payment channels, but ${parkingResearch.length} records still require ownership and charge verification before a definitive Free or Pay label is shown.\n`
    + `- The Eryri route directory was checked as an official research source; missing trails remain an evidence queue rather than being inferred from ordinary mapped paths.\n`
    + `- The live all-town Overpass refresh timed out, so this audit uses the complete cached OSM source snapshot retained by the Gwynedd generator.\n\n`
  ...
  console.log(`Gwynedd audit: ${report.compliance.hardRulesPassed ? 'PASS' : 'FAIL'}; ${projects.length} towns; ${hardFailureCount} hard failure(s).`);
