...
const projectPath = resolve('data/projects/anstruther.json');
...
async function enrich(feature: HeritageFeature): Promise<'dated' | 'undated' | 'failed'> {
  ...
  try {
    const response = await fetch(url, { headers: { 'User-Agent': 'Townscape Guides Anstruther research/1.0' } });
...
const heritage = pkg.features.filter((feature) => feature.tags.includes('hes-listed-building') && !feature.tags.includes('map-hidden') && feature.evidenceScope !== 'out_of_scope');
...
if (pkg.validation.some((item) => item.severity === 'error')) throw new Error('Anstruther date enrichment introduced validation errors.');
await writeFile(projectPath, `${JSON.stringify(pkg, null, 2)}\n`, 'utf8');
await writeFile(resolve('data/review/anstruther-hes-date-enrichment-2026-08-26.json'), `${JSON.stringify({ reviewedAt, total: heritage.length, ...counts, method: 'Official HES designation description; first defensible construction date or period only.', sourceUrlPattern: 'https://portal.historicenvironment.scot/designation/LB…' }, null, 2)}\n`, 'utf8');
console.log(`Anstruther HES dates: ${counts.dated} dated, ${counts.undated} checked but undated, ${counts.failed} fetch failures.`);
