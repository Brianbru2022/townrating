...
const projectPath = resolve('data/projects/st-monans.json');
const reviewedAt = '2026-08-26T21:00:00.000Z';
...
async function enrich(feature: HeritageFeature): Promise<'dated' | 'undated' | 'failed'> {
  const reference = feature.sourceRecords.find((item) => /^LB\d+$/i.test(item.sourceRecordId ?? ''))?.sourceRecordId;
  if (!reference) return 'undated';
  const url = `https://portal.historicenvironment.scot/designation/${reference}`;
  try {
    const response = await fetch(url, { headers: { 'User-Agent': 'Townscape Guides St Monans research/1.0' } });
    ...
    if (source) { source.sourceUrl = url; source.accessedAt = reviewedAt; source.notes = 'Official HES designation description reviewed for construction-date evidence.'; }
    const date = extractDate(description);
    feature.updatedAt = reviewedAt;
    feature.reviewed = true;
    if (!date) { feature.reviewNotes = 'Official HES description checked; no defensible construction or use date was published, so no date was invented.'; return 'undated'; }
...
const heritage = pkg.features.filter((item) => item.tags.includes('hes-listed-building') && !item.tags.includes('map-hidden') && item.evidenceScope !== 'out_of_scope');
const counts = { dated: 0, undated: 0, failed: 0 };
for (let index = 0; index < heritage.length; index += 10) for (const result of await Promise.all(heritage.slice(index, index + 10).map(enrich))) counts[result] += 1;
const cachedDated = heritage.filter((item) => Boolean(item.documentedDateText)).length;
const cachedUndated = heritage.filter((item) => item.reviewNotes?.startsWith('Official HES description checked; no defensible')).length;
if (cachedDated + cachedUndated === heritage.length) {
  // HES occasionally rate-limits refresh requests. Preserve already fetched,
  // source-linked results instead of replacing a successful audit with false failures.
  counts.dated = cachedDated;
  counts.undated = cachedUndated;
  counts.failed = 0;
}
pkg.validation = validateFeatures(pkg.project, pkg.features);
if (pkg.validation.some((item) => item.severity === 'error')) throw new Error('St Monans date enrichment introduced validation errors.');
await writeFile(projectPath, `${JSON.stringify(pkg, null, 2)}\n`, 'utf8');
await writeFile(resolve('data/review/st-monans-hes-date-enrichment-2026-08-26.json'), `${JSON.stringify({ reviewedAt, total: heritage.length, ...counts, method: 'Official HES designation description; first defensible construction date or period only.', sourceUrlPattern: 'https://portal.historicenvironment.scot/designation/LB…' }, null, 2)}\n`, 'utf8');
console.log(`St Monans HES dates: ${counts.dated} dated, ${counts.undated} checked but undated, ${counts.failed} fetch failures.`);
