...
const batchAuditPath = resolve('data/review/leicestershire-settlement-batch-audit-2026-08-11.json');
const dateAuditPath = resolve('data/review/leicestershire-nhle-date-enrichment-2026-08-11.json');
const jsonOutputPath = resolve(`data/review/leicestershire-full-completeness-audit-${reviewedDate}.json`);
const markdownOutputPath = resolve(`data/review/leicestershire-full-completeness-audit-${reviewedDate}.md`);
...
async function main() {
  ...
  for (const { package: projectPackage } of projects) {
    ...
    if (!projectPackage.project.id.endsWith('-leicestershire-england')) {
  ...
  const report = {
    ...
    scope: 'All 157 towns in the bundled Leicestershire settlement manifest.',
    methodology: [
  ...
  const markdown = `# Leicestershire town completeness and compliance audit\n\n`
  ...
  console.log(`Leicestershire audit: ${report.compliance.hardRulesPassed ? 'PASS' : 'FAIL'}; ${projects.length} towns; ${hardFailureCount} hard failure(s).`);
