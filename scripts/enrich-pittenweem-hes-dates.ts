...
const projectPath = resolve('data/projects/pittenweem.json');
const reviewedAt = '2026-08-26T17:00:00.000Z';
...
async function enrich(feature: HeritageFeature): Promise<'dated' | 'undated' | 'failed'> {
  ...
  try {
    const response = await fetch(url, { headers: { 'User-Agent': 'Townscape Guides Pittenweem research/1.0' } });
...
if (pkg.validation.some((item) => item.severity === 'error')) throw new Error('Pittenweem date enrichment introduced validation errors.');
await writeFile(projectPath, `${JSON.stringify(pkg, null, 2)}\n`, 'utf8');
await writeFile(resolve('data/review/pittenweem-hes-date-enrichment-2026-08-26.json'), `${JSON.stringify({ reviewedAt, total: heritage.length, ...counts, method: 'Official HES designation description; first defensible construction date or period only.', sourceUrlPattern: 'https://portal.historicenvironment.scot/designation/LB…' }, null, 2)}\n`, 'utf8');
console.log(`Pittenweem HES dates: ${counts.dated} dated, ${counts.undated} checked but undated, ${counts.failed} fetch failures.`);
