...
const projectPath = resolve('data/projects/elie.json');
const reviewedAt = '2026-08-26T22:00:00.000Z';
...
async function enrich(feature: HeritageFeature): Promise<'dated' | 'undated' | 'failed'> {
  ...
  try {
    const response = await fetch(url, { headers: { 'User-Agent': 'Townscape Guides Elie research/1.0' } });
...
if (cachedDated + cachedUndated === heritage.length) { counts.dated = cachedDated; counts.undated = cachedUndated; counts.failed = 0; }
pkg.validation = validateFeatures(pkg.project, pkg.features);
if (pkg.validation.some((item) => item.severity === 'error')) throw new Error('Elie date enrichment introduced validation errors.');
await writeFile(projectPath, `${JSON.stringify(pkg, null, 2)}\n`, 'utf8');
await writeFile(resolve('data/review/elie-hes-date-enrichment-2026-08-26.json'), `${JSON.stringify({ reviewedAt, total: heritage.length, ...counts, method: 'Official HES designation description; first defensible construction date or period only.', sourceUrlPattern: 'https://portal.historicenvironment.scot/designation/LB…' }, null, 2)}\n`, 'utf8');
console.log(`Elie HES dates: ${counts.dated} dated, ${counts.undated} checked but undated, ${counts.failed} fetch failures.`);
