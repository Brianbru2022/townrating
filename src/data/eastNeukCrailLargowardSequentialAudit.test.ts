...
import hesCertification from '../../data/review/east-neuk-crail-largoward-hes-date-certification-2026-09-02.json';
import linkCheck from '../../data/review/east-neuk-crail-largoward-published-link-check-2026-09-02.json';
import sequentialAudit from '../../data/review/east-neuk-crail-largoward-sequential-audit-summary-2026-09-02.json';
...
const expected = [
  ...
  ['craighead-crail-scotland', 42, 1, 0, 1, 0, 1, 0, false],
  ...
  ['kilconquhar-scotland', 65, 3, 0, 1, 0, 1, 0, true],
...
describe('Balcomie-to-Largoward sequential full audit', () => {
  it('keeps all 22 places selectable and maps only independently qualifying 60+ settlements', () => {
  ...
  it('keeps the Fife Kingsmuir and Lochty records distinct from similarly named places elsewhere', () => {
  ...
  it('retains the complete local HES/NRHE set while exposing only materially dated heat pins', () => {
    expect(hesCertification.projects).toHaveLength(22);
  ...
  it('records an explicit dog-policy result for every published See, Eat and Trail place', () => {
    for (const [id] of expected) {
      const pkg = project(id);
      const curation = publishedPlannerCurationForProject(id);
  ...
  it('does not contain an unexplained exact-58 score in this batch', () => {
    expect(expected.filter(([, score]) => Number(score) === 58)).toEqual([]);
    expect(sequentialAudit.audits.every((audit) => audit.score !== 58 || audit.heritage.visiblePinsWithoutDates === 0)).toBe(true);
  });

  it('checks every published link, including all 18 trail destinations', () => {
    expect(linkCheck.totals).toMatchObject({ checked: 61, failed: 0, trailsChecked: 18, trailsFailed: 0 });
  });
});
